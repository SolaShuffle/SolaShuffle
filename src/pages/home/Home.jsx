		...
		if (!user.homeSignature) {
			(async () => {
				try {
					...
					const resp = await axios.get(
						...
						}/rooms/creator/${wallet.publicKey.toBase58()}?signature=${signature}`
					);
					setUser((prevData) => ({
						...prevData,
			...
			(async () => {
				const resp = await axios.get(
					`${
						import.meta.env.VITE_API
					}/rooms/creator/${wallet.publicKey.toBase58()}?signature=${
						user.homeSignature
					}`
				);
				setMyRooms(resp.data);
	...
	return (
		...
		>
			<div className="flex w-full flex-col">
				...
				<div className="w-full overflow-y-visible px-4 sm:px-3 lg:px-2">
					<div className="relative w-full overflow-x-hidden overflow-y-visible">
						<motion.div
							...
							className="overflow-y-visible whitespace-nowrap py-6"
						>
				...
				<div className="w-full overflow-y-visible px-4 sm:px-3 lg:px-2">
					<div className="relative w-full overflow-x-hidden overflow-y-visible">
						<motion.div className="overflow-y-visible whitespace-nowrap py-6">
							{myRooms.length > 0
								? myRooms.map((room) => {
										return (
											<div
